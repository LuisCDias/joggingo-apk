		...
		start_track.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if(!name.matches("")){
					...
					track_id = db.addTrack(new Track(track_name.getText().toString(),"Porto", "Portugal", 1, 1,0,initial_time));
					Log.d("Track no:",track_id+"");
				}
				else{
					Log.d("Track name: ", track_name.getHint().toString());
					track_id = db.addTrack(new Track(track_name.getHint().toString(),"Porto", "Portugal", 1, 1,0, initial_time));
					Log.d("Track no:",track_id+"");
				}
